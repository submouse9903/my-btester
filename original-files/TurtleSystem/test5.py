...
class TurtleSystem:
    ...
    @timer_decorator #여기서 데이터를 불러오는 것임. 55개에 30초중 5초가 걸림
    def _get_data(self):
    ...
    def _calc_breakouts(self):
        ...
        newframe = self.data.copy()

        # 각 티커에 대해 계산을 수행합니다.
        for t in self.tickers:
            # 롤링 최대값과 최소값을 계산합니다.
            newframe[t, 'S1_EL'] = newframe[t]['Close'].rolling(self.sys1_entry).max()
            newframe[t, 'S1_ExL'] = newframe[t]['Close'].rolling(self.sys1_exit).min()
            newframe[t, 'S2_EL'] = newframe[t]['Close'].rolling(self.sys2_entry).max()
            newframe[t, 'S2_ExL'] = newframe[t]['Close'].rolling(self.sys2_exit).min()

            if self.shorts:
                newframe[t, 'S1_ES'] = newframe[t]['Close'].rolling(self.sys1_entry).min()
                newframe[t, 'S1_ExS'] = newframe[t]['Close'].rolling(self.sys1_exit).max()
                newframe[t, 'S2_ES'] = newframe[t]['Close'].rolling(self.sys2_entry).min()
                newframe[t, 'S2_ExS'] = newframe[t]['Close'].rolling(self.sys2_exit).max()
        self.data = newframe

    @timer_decorator #55개 30초에 2초 걸림
    def _calc_N(self):
        # Calculates N for all tickers
        tr_dict = {}
        for t in self.tickers:
            tr = self.data[t].apply(lambda x: calcTR(x['High'], x['Low'], x['Close']), axis=1)
            tr_dict[t] = tr.rolling(self.atr_periods).mean()

        # 그런 다음 모든 열을 한 번에 DataFrame에 추가합니다.
        for t, tr in tr_dict.items():
            self.data[t, 'N'] = tr
    ...
    @timer_decorator #병렬화가 필요한 곳, 실질적으로 55개 30초에서 30-7 23초정도 걸림
    def run(self):
...
syms = df['Symbol']
# Sample symbols
tickers = list(np.random.choice(syms.values, size=55))
print("Ticker Symbols:")
_ = [print(f"\t{i}") for i in tickers]

# 여기서 백테스팅이 이루어짐, 여기 병렬화로 처리해야 빠를텐데
